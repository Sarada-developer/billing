...
class Customers extends MY_Controller
{
    ...
    public function redeem_points($company_id = null)
    {
        ...
        $this->form_validation->set_rules('points', lang('points'), 'required|numeric');
        if ($this->form_validation->run() == true) {
            if ($this->Owner || $this->Admin) {
                $date = $this->sma->fld(trim($this->input->post('date')));
            } else {
            ...
            $cdata = [
                'award_points' => ($company->award_points - $this->input->post('points')),
            ];

            // $this->sma->redeem_point($company_id,$this->input->post('points'));


           // print_r($cdata);
            //die();
            
        }   elseif ($this->input->post('points')) {
            $this->session->set_flashdata('error', validation_errors());
            //admin_redirect('customers');
        }
        ...
        if ($this->form_validation->run() == true) {
            ...
            //admin_redirect('customers');
           //print_r($cdata);
           $this->sma->redeem_point($this->input->post('points'),$company->id);
           //die('f');
           admin_redirect('customers');
            ...
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['company']  = $company;
            $this->data['points'] = $this->companies_model->getCustomerPointsByCustomer($company_id)->award_points;
            $this->load->view($this->theme . 'customers/redeem_award_point', $this->data);
        }
    }
