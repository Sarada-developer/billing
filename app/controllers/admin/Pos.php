...
class Pos extends MY_Controller
{
    ...
    public function getSales($warehouse_id = null)
    {
        ...
        if ($warehouse_id) {
            ...
            $this->datatables
                ->select($this->db->dbprefix('sales') . ".id as id, DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, biller, customer, (grand_total+COALESCE(rounding, 0)), paid, CONCAT(grand_total, '__', rounding, '__', paid) as balance, sale_status, payment_status, ($ratio * {$this->db->dbprefix('sales')}.grand_total) ,companies.email as cemail")
                ->from('sales')
                ->join('companies', 'companies.id=sales.customer_id', 'left')
                ->group_by('sales.id');
        }
    ...
    public function index($sid = null)
    {
        ...
        if ($this->form_validation->run() == true && !empty($products) && !empty($data)) {
            ...
            $this->data['ratio']           = $this->pos_model->getRatio();
