...
class Reports extends MY_Controller
{
    ...
    public function getPointReport($pdf = null, $xls = null)
    {
        $this->sma->checkPermissions('payments', true);
        ...
        if ($pdf || $xls) {
            ...
            $sql = ("{$this->db->dbprefix('point_history')}.date as date, " . $this->db->dbprefix('point_history') . '.type as type, ' . $this->db->dbprefix('point_history') . '.points as point, ' . $this->db->dbprefix('point_history') . ".remarks as remarks");
            if ($customer) {
                $this->datatables->where('point_history.customer_id', $customer);
               // echo  "c" . $customer;
            }
            $this->datatables
                ->select($sql)   
