...
class Sma
{
    ...
    public function update_award_points($total, $customer, $user = null, $scope = null, $ref_no = null) 
    {
        if (!empty($this->Settings->each_spent) && $total >= $this->Settings->each_spent) {
            ...
            $pdata['date']  = date($this->dateFormats['php_ldate']);
            $pdata['customer_id']  = $customer;
            $pdata['type']  = 'Earned';
            $pdata['points']  = $points;
            $pdata['remarks']  = "Earned from Sale  ".$ref_no;

            $this->db->insert('point_history', $pdata);
            ///Award History 
        }
    ...
    public function redeem_point($points, $customer){
